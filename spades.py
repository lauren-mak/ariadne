...
def fill_cfg(options_to_parse, log, secondary_filling=False):
        ...
        # UPDATE - New options for Ariadne deconvolution
            ...
            if support.is_int(arg) and int(arg) >= 0:
                ...
                support.error('Incorrect value for --size-cutoff: ' + arg +
                              ' (should be a positive int number)', log)
        elif opt == "--assembly-graph":
            options_storage.assembly_graph = arg
            options_storage.run_assembly = False
    ...
    if not options_storage.only_error_correction:
        ...
        cfg["assembly"].__dict__["assembly_graph"] = options_storage.assembly_graph
        cfg["assembly"].__dict__["run_assembly"] = options_storage.run_assembly
